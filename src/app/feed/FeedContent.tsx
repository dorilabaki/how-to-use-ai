    ...
    <Section className="pt-24">
      ...
      <div className="max-w-3xl mx-auto">
          ...
          {feedPosts.map((post, i) => (
            ...
            >
              ...
              <div className="flex items-center gap-4 mb-4">
                <div className="w-12 h-12 rounded-full bg-gradient-to-br from-primary-500 to-accent-500 flex items-center justify-center text-white font-bold" aria-hidden="true">
                  AI
                </div>
                <div>
              ...
              <div className="flex items-center gap-6 pt-4 border-t border-dark-600 text-sm text-text-muted">
                <span className="flex items-center gap-2">
                  ...
                  <span><span className="sr-only">Likes: </span>{post.likes.toLocaleString()}</span>
                </span>
                <span className="flex items-center gap-2">
                  <svg className="w-4 h-4" aria-hidden="true" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                    <path strokeLinecap="round" strokeLinejoin="round" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" />
                  </svg>
                  <span><span className="sr-only">Comments: </span>{post.comments.toLocaleString()}</span>
                </span>
                {post.reposts && (
                  <span className="flex items-center gap-2">
                    <svg className="w-4 h-4" aria-hidden="true" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                    ...
                    <span><span className="sr-only">Reposts: </span>{post.reposts.toLocaleString()}</span>
                  </span>
                )}
                <a
        ...
        <div className="text-center">
          ...
          <Button href={feedConfig.linkedInUrl} external>
